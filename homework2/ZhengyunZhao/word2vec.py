...
def negSamplingLossAndGradient(
    ...
    y_hat = sigmoid(np.dot(centerWordVec, outsideVectors[outsideWordIdx]))
    loss -= np.log(y_hat)
    
    gradOutsideVecs[outsideWordIdx] += centerWordVec * (y_hat - 1)
    gradCenterVec += outsideVectors[outsideWordIdx] * (y_hat - 1)
    
    neg = outsideVectors[negSampleWordIndices]
    y_hat = sigmoid(-np.dot(neg, centerWordVec))
    loss -= np.sum(np.log(y_hat))
    
    for k in range(K):
        temp = indices[k+1]
        z = sigmoid(-np.dot(outsideVectors[temp], centerWordVec))
        gradOutsideVecs[temp] -=  centerWordVec * (z - 1.0)
        gradCenterVec -= outsideVectors[temp] * (z - 1.0)
    ### END YOUR CODE
