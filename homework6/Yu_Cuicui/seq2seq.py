...
class LSTM(nn.Module):
    ...
    def forward(self, input, hidden,cell):
        ...
        i_t = torch.sigmoid(torch.mm(self.w_ii, input) + self.b_i + torch.mm(self.w_hi, hidden))
        f_t = torch.sigmoid(torch.mm(self.w_if, input) + self.b_f + torch.mm(self.w_hf, hidden))
        g_t = torch.sigmoid(torch.mm(self.w_ig, input) + self.b_g + torch.mm(self.w_hg, hidden))
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_output):
        ...
        output, hidden, cell = self.lstm(embed, hidden, encoder_output)
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for i in range(input_length):
        ...
        # 这里这样取是为了使用attention方便，不使用attention可直接使用 encoder_outputs = encoder_hidden[0]
    ...
    for i in range(target_length):
        decoder_output, decoder_hidden = decoder(decoder_input, decoder_hidden, encoder_outputs[input_length-1])
