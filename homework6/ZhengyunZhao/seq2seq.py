...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.W_in = nn.Parameter(torch.Tensor(self.hidden_size, self.input_size))
        ...
        self.W_hn = nn.Parameter(torch.Tensor(self.hidden_size, self.hidden_size))
        self.b_ir = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.b_iz = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.b_in = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.b_hr = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.b_hz = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.b_hn = nn.Parameter(torch.Tensor(self.hidden_size, 1))
        self.initial_weights()
        
    def forward(self, input, hidden):
        input = input.reshape(-1, 1)
        hidden = hidden.reshape(-1, 1)
        reset = torch.sigmoid(torch.mm(self.W_ir, input) + self.b_ir + 
                          torch.mm(self.W_hr, hidden) + self.b_hr)
        update = torch.sigmoid(torch.mm(self.W_iz, input) + self.b_iz + 
                          torch.mm(self.W_hz, hidden) + self.b_hz)
        new = torch.tanh(torch.mm(self.W_in, input) + self.b_in + reset.mul(
                torch.mm(self.W_hn, hidden) + self.b_hn))
...
class DecoderAtt(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.embedding = nn.Embedding(output_size, hidden_size)
        self.attn_w = nn.Linear(self.hidden_size * 2, self.max_length)
        self.attn_l = nn.Linear(self.hidden_size * 2, self.hidden_size)
        self.gru = GRU(hidden_size, hidden_size)
        self.linear = nn.Linear(hidden_size, output_size)
        self.softmax = nn.LogSoftmax(dim = 1)
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        embed = self.embedding(input).view(1,1,-1)
        embed = self.dropout(embed)
        attn_weights = self.attn_w(torch.cat((embed[0], hidden[0]), dim = 1))
        attn_weights = F.softmax(attn_weights, dim = 1)
        attn = torch.mm(attn_weights, encoder_outputs)
        attned = self.attn_l(torch.cat((attn, embed[0]), dim = 1))
        attned = F.relu(attned)
        output, hidden = self.gru(attned, hidden)
        output = self.linear(output[0])
        log_softmax = self.softmax(output)
        return log_softmax, hidden
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for i in range(target_length):
        ...
        pred = decoder_output.data.topk(1)[1]
...
def main():
    ...
    decoder = DecoderAtt(args.hidden_size, tgt_vocab.n_words, dropout_p=0.1).to(device)
