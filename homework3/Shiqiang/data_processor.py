...
def tokenizer(text): 
    ...
    filtered_str = filtrate.sub(r'', text)#replace
    #words = [word for word in jieba.cut(filtered_str)]
...
def get_stop_words():
    ...
    for stop_word in data:
        stop_word = bytes.decode(stop_word)    # str(file_object.readlines(), encoding = "utf8")
        temp1 = stop_word.strip() # strip():默认删除空白符（包括 ‘\n’, ‘\r’, ‘\t’, ’ ')
        stop_words.append(temp1)
    '''
    
    stop_words = [bytes.decode(line).strip() for line in file_object.readlines()]
